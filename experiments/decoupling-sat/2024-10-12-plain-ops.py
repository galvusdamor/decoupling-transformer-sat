...
exp.add_fetcher("data/2024-09-03-fix-no-global-op-eval", name='fetch-others', filter=[filter_baseline_algorithms], merge=True)
...
exp.add_report(AbsoluteReport(attributes=attributes, filter=[factoring_filter.add_runs, factoring_filter.filter_non_decoupled_runs, filters.remove_revision, filter_kissat_oom, virtual_solver_filter.add_run, virtual_solver_filter.replace_config], filter_algorithm=["blind", "blind-LP-F0.2s1M", 
                                                "sat", "sat-LP-F0.2s1M", 
                                                "sat-inc-", "sat-inc-LP-F0.2s1M",
                                                "ff", "ff-LP-F0.2s1M"]), outfile=f"{SCRIPT_NAME}-all.html")

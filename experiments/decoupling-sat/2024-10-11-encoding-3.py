...
REVISION = "b4c39ff43d8d887bbc40b1dad40122e5390d3fa0"
REVISIONS = [REVISION]
...
factoring_filter_mob = filters.NonDecoupledTaskFilter(["aEsat-LP-F0.2s1M-clo"])

exp.add_report(AbsoluteReport(attributes=attributes, filter=[filters.remove_revision, factoring_filter_mob.add_runs, factoring_filter_mob.filter_non_decoupled_runs, filter_kissat_oom]), outfile=f"{SCRIPT_NAME}-mobility-all.html")

factoring_filter_leaf = filters.NonDecoupledTaskFilter(["aEsat-LP-L0.8s1M-clo"])

exp.add_report(AbsoluteReport(attributes=attributes, filter=[filters.remove_revision, factoring_filter_leaf.add_runs, factoring_filter_leaf.filter_non_decoupled_runs, filter_kissat_oom]), outfile=f"{SCRIPT_NAME}-leaves-all.html")
